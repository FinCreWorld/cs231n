...
def svm_loss(x, y):
    ...
    margin = x - correct_class_score.reshape(-1, 1) + 1
    margin[margin < 0] = 0
...
def softmax_loss(x, y):
    ...
    N = x.shape[0]
    scores = x - np.max(x, axis=1).reshape(-1, 1) # Attention: x can not be modified
    expx = np.exp(scores)
    sumex = np.sum(expx, axis=1)

    loss = -np.sum(np.log(expx[np.arange(N), y] / sumex))
    loss /= N
    
    dx = expx / sumex.reshape(-1, 1)
    dx[np.arange(N), y] -= 1
    dx /= N
