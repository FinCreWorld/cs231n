...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, _, H = *x.shape, b.shape[0]
    h = np.zeros(shape=(N, T, H))
    for t in range(0, T):
        h[:, t, :], _ = rnn_step_forward(x[:, t, :], h[:, t-1, :] if t > 0 else h0, Wx, Wh, b)
    cache = (x, h0, h, Wx, Wh)
...
def rnn_backward(dh, cache):
    ...
    x, h0, h, Wx, Wh = cache
    dx, dWx, dWh, db = 0, 0, 0, 0
    T = x.shape[1]
    dx = np.zeros_like(x)
    for t in range(T-1, -1, -1):
        dx[:, t, :], ddh, ddWx, ddWh, ddb = rnn_step_backward(dh[:, t, :] + (ddh if t < T-1 else 0), (x[:, t, :], h[:, t-1, :] if t > 0 else h0, Wx, Wh, h[:, t, :]))
        dWx += ddWx
        dWh += ddWh
        db += ddb
    dh0 = ddh
...
def word_embedding_backward(dout, cache):
    ...
    W_shape, x = cache
    dW = np.zeros(W_shape)
    # print(dW.shape, x.shape, dout.shape)
    np.add.at(dW, x, dout)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_h, prev_c, Wx, Wh, i, f, o, g, tanh_next_c)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = [None] * T
    for t in range(T):
        h[:, t, :], c, (_, _, _, _, _, i, f, o, g, tanh_next_c) = lstm_step_forward(x[:, t, :], h[:, t-1, :] if t > 0 else h0, c, Wx, Wh, b)
        c_list.append(c)
        ifog_list.append((i, f, o, g))
        tanh_next_c_list.append(tanh_next_c)

    cache = (x, h0, Wx, Wh, h, c_list, ifog_list, tanh_next_c_list)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_backward(dh, cache):
    ...
    dWx, dWh, db = 0, 0, 0
    x, h0, Wx, Wh, h, c_list, ifog_list, tanh_next_c_list = cache
    T = x.shape[1]
    dc = np.zeros_like(h0)
    dx = np.zeros_like(x)
    ddprev_h = 0
    for t in range(T-1, -1, -1):
        ...
        prev_c = c_list[t-1] if t > 0 else np.zeros_like(h0)
        i, f, o, g = ifog_list[t]
        dx[:, t, :], ddprev_h, dc, ddWx, ddWh, ddb = lstm_step_backward(dh[:, t, :] + ddprev_h, dc, (x[:, t, :], prev_h, prev_c, Wx, Wh, i, f, o, g, tanh_next_c_list[t]))
        dWx += ddWx
        dWh += ddWh
        db += ddb
    dh0 = ddprev_h
