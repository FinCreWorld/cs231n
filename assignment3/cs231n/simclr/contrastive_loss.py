...
def sim(z_i, z_j):
    ...
    norm_dot_product = torch.dot(z_i, z_j) / torch.norm(z_i, 2) / torch.norm(z_j, 2)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_pair = torch.exp(sim(z_k, z_k_N) / tau)
        sim_k_sum = 0
        sim_k_N_sum = 0
        for i in range(2*N):
            sim_k_sum += torch.exp(sim(z_k, out[i]) / tau)
            sim_k_N_sum += torch.exp(sim(z_k_N, out[i]) / tau)
        sim_k_sum -= torch.exp(torch.tensor(1 / tau))
        sim_k_N_sum -= torch.exp(torch.tensor(1 / tau))
        total_loss += -torch.log(sim_pair / sim_k_sum)
        total_loss += -torch.log(sim_pair / sim_k_N_sum)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.sum(out_left * out_right, dim=1) / torch.norm(out_left, p=2, dim=1) / torch.norm(out_right, p=2, dim=1)
...
def compute_sim_matrix(out):
    ...
    out1 = out.view(N, 1, D)
    out2 = out.view(1, N, D)
    sim_matrix = torch.sum(out1 * out2, dim=2) / torch.linalg.norm(out1, dim=2) / torch.linalg.norm(out2, dim=2)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pair = sim_positive_pairs(out_left, out_right).to(device)
    ...
    numerator = torch.exp(sim_pair / tau)

    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss_matrix1 = torch.log(numerator / denom[:N].view(-1, 1))
    loss_matrix2 = torch.log(numerator / denom[N:].view(-1, 1))
    loss = -1/(2*N)*torch.sum(loss_matrix1 + loss_matrix2)
